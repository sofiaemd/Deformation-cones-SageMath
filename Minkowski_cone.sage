...
def Minkowski_cone(P):
    """
    INPUT: Polytope
    OUTPUT: Tuple containing the cone parametrizing weak Minkowski summands, the list of edges of P in order and the list of vertices of P.
...
def build_summand_mink(C,edges_p,vertices_p, build_ray):
    ...
    C_ray=[(1/max(C_ray))*float(c) for c in C_ray]
    dict_edges={
        vertices_p[0]: vector(vertices_p[0])
    }
